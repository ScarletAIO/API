...
export default class DataHandler {
	...
	async addUserToTable(userData: {
		...
		const query: string = `INSERT INTO \`user.table\` (age, name, password, email) VALUES (${age}, '${name}', '${password}', '${email}');`;
	...
	async updateUserInTable(userData: any | {}): Promise<any> {
		...
		const query: string = `UPDATE \`user.table\` SET age = ${age}, name = '${name}', password = '${password}', email = '${email}' WHERE id = '${userData.id}';`;
		const result: any = await connection.query(query);
		connection.end();
		return result;
	}
	...
	async deleteUserFromTable(userData: any | {}): Promise<any> {
		...
		const query: string = `DELETE FROM \`user.table\` WHERE id = '${userData.id}';`;
		const result: any = await connection.query(query);
		connection.end();
		return result;
	}
	...
	async getUserFromTable(userData: any | {}): Promise<any> {
		...
		const query: string = `SELECT * FROM \`user.table\` WHERE id = '${userData.id}';`;
		const result: any = await connection.query(query);
		connection.end();
		return result;
	}
	...
	async getUserFromTableByParam(
		...
		const connection: any = await this.createConnection();
		// get the user data from the user.table
		const query: string = `SELECT * FROM \`user.table\` WHERE ${param} = '${userData[param]}';`;
		const result: any = await connection.query(query);
		connection.end();
		return result;
	}
